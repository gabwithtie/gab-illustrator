    ...
    void NoteEditingControls::HandleKeyboardShortcuts(gsr::App &app, Model::Clip &clip)
    {
        ...
        if (!io.KeyCtrl)
        {
            ...
            if (s_strum.Begin(io.KeyShift && ImGui::IsKeyDown(ImGuiKey_G)))
            {
                ...
                // --- 2. Active Session Updates ---
                int64_t step_delta = 0;
                if (ImGui::IsKeyPressed(ImGuiKey_RightArrow))
                    step_delta += grid_snap_ticks;
                ...
                return; // Intercept inputs while shortcut is active
            }
            ...
            if (s_move.Begin(io.KeyShift && ImGui::IsKeyDown(ImGuiKey_T)))
            {
                ...
                if (up || down || left || right)
                {
                    ...
                    for (auto &note : clip.notes)
                    {
                        ...
                        if (up)
                            note.pitch = static_cast<uint8_t>(std::clamp(note.pitch + 1, 0, 127));
                        if (down)
                            note.pitch = static_cast<uint8_t>(std::clamp(note.pitch - 1, 0, 127));
                ...
                return; // Block standard arrow inputs while holding Shift + T
            }
            ...
            if (s_scale.Begin(io.KeyShift && ImGui::IsKeyDown(ImGuiKey_S)))
            {
                ...
                if (up || down || left || right)
                {
                    ...
                    for (auto &note : clip.notes)
                    {
                        ...
                        if (up)
                            note.pitch = static_cast<uint8_t>(std::clamp(note.pitch + 12, 0, 127));
                        if (down)
                            note.pitch = static_cast<uint8_t>(std::clamp(note.pitch - 12, 0, 127));
                ...
                return; // Block standard arrow inputs while holding Shift + S
            }
        }
    }
